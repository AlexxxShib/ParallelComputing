...
struct Solver
{
	...
	void solve()
	{
		...
		{
			if (i == rank)
				for (int j = sizes[i] - 1; j >= 0; j--, nCol--)
				{
					_x[j] = _A[j][N];
					MPI_Bcast(&_x[j], 1, MPI_DOUBLE, i, MPI_COMM_WORLD);
					for (int k = 0; k < j; k++)
						_A[k][N] -= _A[k][nCol] * _x[j];
				}
			else 
			{
				double x_ncol;
				for (int k = 0; k < sizes[i]; k++, nCol--)
				{
					MPI_Bcast(&x_ncol, 1, MPI_DOUBLE, i, MPI_COMM_WORLD);
					if (rank < i)
						for (int l = 0; l < sizes[rank]; l++)
							_A[l][N] -= _A[l][nCol] * x_ncol;
				}
			}
		}
	}
